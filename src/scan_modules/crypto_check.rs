...
pub struct CryptoCheckScanModule;

impl ScanModule for CryptoCheckScanModule {
	fn scan(&self, scan_object: &ScanObject) -> Result<Vec<Finding>, ArmorlibError> {
		...
		let stringtext = match scan_object.get_metadata("text/text") {
			Ok(meta) => meta,
				Err(err) => {
					return Err(MissingMetadata(format!("Unable to get metadata text/text. Error: {}",err)));
				}
		};
		let api_count;
		let kw_count; 

		api_count = api_check(stringtext.trim());
		kw_count = keyword_check(stringtext.trim());
		if api_count >= 5 {

			findings.push(Finding {
											title: String::from("Found:"),
											description: format!("Identified {} calls", api_count),
											id: String::from("Located Cryptographic function calls"),
											severity: Severity::Warn(String::from("Identified over 5 function calls"))
									});
		}
		...
		if kw_count >= 10 {
			...
			findings.push(Finding {
											title: String::from("Found:"),
											description: format!("Identified {} keywords", kw_count),
											id: String::from("Located Cryptographic keywords"),
											severity: Severity::Warn(String::from("Identified over 10 keywords"))
									});                
		}
	...
	fn info(&self) -> (&'static str, &'static str) {
		("crypto_check","Identifies cryptographic keywords and api function calls")
	}
...
fn api_check(fun: &str) -> usize {
	...
	let mut count = 0;
	for i in arr.iter() {
		if fun.contains(i) { count+= 1; }
	}
	count

} 

fn keyword_check(kw: &str) -> usize {
