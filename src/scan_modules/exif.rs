...
use ArmorlibError::UnknownProcessingError;
    ...
    fn scan(&self, scan_object: &ScanObject) -> Result<Vec<Finding>, ArmorlibError> {
        ...
        for field in fields {
            ...
            let value: &Value = &field.value;
						...
						let tag_default_val = match tag.default_value() {
							Some(dv) => dv,
								None => {
									return Err(UnknownProcessingError(String::from("Unable to get tag default value. Error: {}")));
									
								}
						};
            if tag.default_value().is_some() {
                if format!("{}", value.display_as(*tag))
                    == format!("{}", tag_default_val.display_as(*tag))
                {
