...
pub fn process(
    ...
    _extra_preprocessors: Vec<Box<Preprocessor>>,
    ...
    for preprocessor in available_preprocessors {
        ...
        {
						//println!("preprocessor to be ran : {}",preprocessor.to_string());
            preprocessors_to_be_run.push(preprocessor);
        }
    }
		//println!("preproc to be run: {}",preprocessors_to_be_run.len());
...
mod tests {
    ...
    #[test]
    fn test_full_cycle() {
				let vec = match util::hex_to_vec("48 8B CD E8 60 FF FF FF 48 FF C3 32 44 1E FF 48 FF CF 88 43 FF 48 8B CD E8 60 FF FF FF 48 FF C3 32 44 1E FF 48 FF CF 88 43 FF 48 8B CD E8 60 FF FF FF 48 FF C3 32 44 1E FF 48 FF CF 88 43 FF") {
					Ok(vec) => vec,
						Err(err) => {
            ...
            match process(
                ...
                binary_object::BinaryObject::from(vec),
                    None
							...
							Ok(p) => p,
								Err(err) => {
									println!("Unable to execute process function. Error: {}",err);
									return;
								}
						};
    }
