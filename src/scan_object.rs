    ...
    fn test_get_metadata() {
        ...
        let scan_object = ScanObject {
            ...
            binary_object: BinaryObject::from(vec![]),
				...
				let m_1_k = match scan_object.get_metadata("module1/key") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for module1/key. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
            ...
            &String::from("m1::value")
				...
				let m_1_k_2 = match scan_object.get_metadata("module1/key2") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for module1/key2. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
            ...
            &String::from("m1::value2")
				...
				let m_2_k_3 = match scan_object.get_metadata("module2/key3") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for module2/key3. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
            ...
            &String::from("m2::value3")
				...
				let m_2_k_2 = match scan_object.get_metadata("module2/key2") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for module2/key2. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
            ...
            &String::from("m2::value2")
				...
				let f_m_k_2 = match scan_object.get_metadata("fakemodule/key2") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for fakemodule/key2. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
            ...
            Err(ArmorlibError::MissingPreprocessor(String::from(
				...
				let m_2_k_4 = match scan_object.get_metadata("module2/key4") {
					Ok(meta) => meta,
						Err(err) => {
							println!("Unable to get metadata for module2/key4. Error: {}",err);
							return;
						}
				}; 
        assert_eq!(
