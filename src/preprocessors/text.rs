...
#[test]
	fn test_utf16_extraction() {
		let text = "Dr. Z requires tests, and so do I";
		let utf16_bytes: Vec<u8> = match util::hex_to_vec("44 72 2e 20 5a 20 72 65 71 75 69 72 65 73 20 74 65 73 74 73 2c 20 61 6e 64 20 73 6f 20 64 6f 20 49") {
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
				}
		};
...
#[test]
	fn test_ascii_extraction() {
		let text = "Dr. Z requires tests, and so do I";
		let ascii_bytes: Vec<u8> = match util::hex_to_vec("44 72 2e 20 5a 20 72 65 71 75 69 72 65 73 20 74 65 73 74 73 2c 20 61 6e 64 20 73 6f 20 64 6f 20 49") {
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
...
#[test]
	fn test_determine_encoding_utf32() {
		// test utf32
		let vec = match util::hex_to_vec("FF FE 00 00 00 00 00") {
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
...
#[test]
	fn test_determine_encoding_ascii() {
		// test ascii
		let vec = match util::hex_to_vec("33 33 33 33 33 33 34 32 12 34"){
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
...
#[test]
	fn test_determine_encoding_utf8() {
		// test utf8
		let vec = match util::hex_to_vec("EF BB BF 00") {
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
...
#[test]
	fn test_determine_encoding_utf16() {
		// test utf16
		let vec = match util::hex_to_vec("FF FE 00 00") {
			Ok(vec) => vec,
				Err(err) => {
					println!("Unable to convert hex to vec. Error: {}",err);
					return;
