...
mod tests {
    ...
    fn test_for_vec() {
        let data: Vec<u8> = vec![]; // empty data for demo
        let _scan_result = match data.process() {
					Ok(sr) => sr,	
						Err(err) => {
							println!("Unable to execute function process for test_for_vec. Error: {}",err);
							return;
						}
				};
    }
    ...
    fn test_for_binary_object() {
        ...
        let _scan_result = match binary_object.process(){
					Ok(sr) => sr,	
						Err(err) => {
							println!("Unable to execute function process for test_for_binary_ojbect. Error: {}",err);
							return;
						}
				};
